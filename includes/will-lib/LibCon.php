...
class LibConException extends GoKabamLibException {}
...
class LibCon {
	...
	public function __construct(string $format, $subtype = null, $detail = null) {
		...
		if ($subtype && (!$b_found)) {
			throw new LibConException("Could not find [$subtype] in the accepted subtype list of " . self::SUBTYPES_SINGLE_ARRAY);
		}
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_json($what): ?string
    {
		if (is_string($what)) {
			...
			try {
				...
				throw new LibConException("Canot convert value to json: ". $bad_json->getMessage());
			}
		} else {
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_float($what): ?float
    {
		if (is_null($what)) {return null;} //nulls stay null
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_text($what): ?string
    {
		if (is_null($what)) {return null;} //nulls stay null
		if (is_object($what)) {
			if ( method_exists( $what, '__toString' ) ) {
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_int($what): ?int
    {
		if (is_numeric($what) || is_bool($what)) {
			$cast =  intval($what);
		}
	 ...
	 * @throws JsonException
	 */
	protected static function convert_to_array($what): array
    {
		if (empty($what) && (!(is_numeric($what) || is_bool($what)))) {
	 ...
	 */
	protected static function convert_var_to_carbon($what,$timezone): Carbon
    {
	...
	protected static function convert_var_to_subtype($what, string $subtype, ?string $detail) {
		...
		switch ($subtype) {
			...
			default: {
				throw new LibConException("Cannot cast subtype, do not recognize [$subtype], it needs to be one of ". self::SUBTYPES_SINGLE_ARRAY);
			}
		}
	}
	 ...
	 * @param string $type
	 *
	 * @return array|bool|bool[]|float|int|int[]|object|string|string[]|null
     * @throws LibConException
	 * @since 0.2.2
	 */
	 ...
	 */
	public static function test($error_level = 0): string
    {
		$types_to_test = [
							'bool',
							'tri',
